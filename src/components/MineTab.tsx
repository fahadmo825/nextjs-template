...
export function MineTab({ onWatch, canWatch, user, pendingEarnings, speedPerHour, loading, onClaim }: MineTabProps) {
  const accountReady = Boolean(user) && !loading;
  const balance = user && !loading ? formatAmount(user.balance) : 'Loading';
  const pending = accountReady ? formatAmount(pendingEarnings) : '--';
  ...
  return <section className="tab-content"><div className="hero-copy"><span className="eyebrow">AGENB MINING NETWORK</span><h1>Make your balance<br /><em>work harder.</em></h1><p>Keep your miner online, stack AGENB, and grow your network.</p></div><div className="balance-card"><div><span className="muted-label">MAIN BALANCE</span><strong>{balance}<span> AGENB</span></strong><small>Level {user?.miningLevel ?? 1} · {speedPerHour} AGENB per hour</small></div><div className="balance-orb"><Zap size={28} /></div></div><div className="pending-card"><div><span className="muted-label">UNCLAIMED MINING EARNINGS</span><strong>{accountReady ? '+' : ''}{pending} <small>AGENB</small></strong><span>Accumulating in real time at {speedPerHour} AGENB/hour</span></div><button className="claim-button claim-primary" onClick={() => void onClaim()} disabled={!accountReady || pendingEarnings <= 0}>CLAIM</button></div><div className="stats-grid">{statItems.map((item) => <div className="stat-card" key={item.label}><span className="stat-icon">{item.icon}</span><span className="muted-label">{item.label}</span><strong>{item.value}</strong></div>)}</div><div className="section-heading"><div><span className="eyebrow">QUICK ACTION</span><h2>Boost your output</h2></div><ArrowUpRight size={20} /></div><div className="boost-panel"><div className="boost-icon"><Play size={20} fill="currentColor" /></div><div><strong>Watch & earn</strong><span>Get +100 AGENB for every reward ad</span></div><button className="primary-button" onClick={onWatch} disabled={!canWatch}><Play size={15} fill="currentColor" /> Watch ad</button></div></section>;
}
